...
const GUEST_CODE_KEY = "guest_access_code";
...
export default function CodeAccessScreen() {
  ...
  const handleAccessWithCode = async () => {
    ...
    try {
      setIsLoading(true);
      // Guardar el código de acceso del invitado
      await AsyncStorage.setItem(GUEST_CODE_KEY, code);
      // Navegar a la pantalla de invitado
      router.push("/guest");
    } catch (error) {
        ...
        <View className="flex-1 gap-8 justify-center">
          ...
          <Card variant="elevated" className="gap-4">
            <View className="gap-2">
              ...
              <Text className="text-xs text-muted">
                Pídele el código al DJ de tu boda
              </Text>
            </View>
          ...
          <View className="bg-primary/10 border border-primary rounded-xl p-4">
            <Text className="text-xs text-primary font-medium">
              💡 El código de 4 dígitos identifica tu boda. Cada pareja tiene su propio código para mantener las peticiones de canciones organizadas.
            </Text>
          </View>
